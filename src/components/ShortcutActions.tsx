...
export default function ShortcutActions({categories, selected_category, setShortcutMode, shortcut_mode}: prop) {
  ...
  <>
    <Flex className="min-w-fit max-w-fit w-1/3 border-[1.9px] rounded-md border-neutral-700 shadow-xl pb-4 pl-4 pr-4 pt-4 flex-col">
      ...
      <Flex className="items-center gap-4">
        <Tooltip label="Create Shortcut">
          <Button onClick={() => create_handler.open()}>
            +
          </Button>
        </Tooltip>
        <Tooltip label="Delete Shortcut">
          <Button variant="outline" onClick={() => ((shortcut_mode == "delete") ? setShortcutMode(""): setShortcutMode("delete"))}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" className="transition-colors"
              color={(shortcut_mode == "delete") ? "#c91a25": "#ffffff"} fill="none">
        ...
        <Tooltip label="Edit Shortcut">
          <Button variant="outline" onClick={() => ((shortcut_mode == "edit") ? setShortcutMode(""): setShortcutMode("edit"))}>
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" className="transition-colors" 
              color={(shortcut_mode == "edit") ? "#f06e27": "#ffffff"} fill="none">
    ...
    <Modal opened={create_opened} onClose={() => create_handler.close()} title="Create Shortcut">
      <ShortcutForm close={create_handler.close} categories={categories} selected_category={selected_category}/>
    </Modal>
  </>
  );
}
